          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
              ...
              <FadeInOnScroll delay={0}>
                <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-8 shadow-sm hover:shadow-lg transition-shadow h-full flex flex-col">
                  <div className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-2">Para descubrir</div>
                  ...
                  <p className="mt-2 text-sm text-[var(--text-secondary)]">Sin tarjeta. Sin compromiso.</p>
                  <ul className="mt-6 space-y-3 text-sm flex-1">
                    {[
                      { text: '1 local', ok: true },
                      { text: 'Diagnóstico de desperdicio', ok: true },
                      { text: 'Plan de prevención personalizado', ok: true },
                      { text: 'Estimación de pérdidas anuales (€)', ok: true },
                      { text: 'Registro y seguimiento continuo', ok: false },
                      { text: 'Alertas de desviación', ok: false },
                    ].map(({ text, ok }) => (
                      <li key={text} className={`flex items-center gap-2 ${ok ? 'text-[var(--text-secondary)]' : 'text-[var(--text-muted)] line-through'}`}>
                        {ok
                          ...
                          : <svg className="h-4 w-4 text-[var(--text-muted)] shrink-0" fill="none" viewBox="0 0 20 20" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        }
                        {text}
                      </li>
                    ))}
                  </ul>
                  <p className="mt-5 text-xs text-[var(--text-muted)] italic">Ideal para entender cuánto dinero estás perdiendo</p>
                  <Link
                    href="/auth/register"
                    className="mt-5 block w-full rounded-lg border border-primary-600 py-3 text-center text-sm font-semibold text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-950 transition-colors"
                  >
                    Empezar gratis
                  </Link>
                </div>
              ...
              <FadeInOnScroll delay={150}>
                <div className="relative rounded-2xl border-2 border-primary-600 bg-[var(--bg-card)] p-8 shadow-lg hover:shadow-xl transition-shadow h-full flex flex-col">
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-primary-600 px-4 py-1 text-xs font-bold text-white whitespace-nowrap">
                    MÁS POPULAR
                  </div>
                  <div className="text-xs font-semibold text-primary-600 uppercase tracking-wider mb-2">Para recuperar dinero</div>
                  <h3 className="text-lg font-semibold text-[var(--text-primary)]">Pro</h3>
                  <div className="mt-3 flex items-baseline gap-1">
                    <span className="text-4xl font-bold text-[var(--text-primary)]">49&euro;</span>
                    <span className="text-[var(--text-muted)]">/mes</span>
                  </div>
                  <p className="mt-2 text-sm text-[var(--text-secondary)]">Hasta 5 locales</p>
                  <div className="mt-3 rounded-lg bg-primary-50 dark:bg-primary-950 px-3 py-2 text-sm font-semibold text-primary-700 dark:text-primary-300">
                    💰 Ahorro típico: 200–800 €/mes
                  </div>
                  <ul className="mt-5 space-y-3 text-sm text-[var(--text-secondary)] flex-1">
                    {[
                      'Todo lo del plan Gratis',
                      'Registro diario de mermas (sin cálculos)',
                      'KPIs automáticos semana a semana',
                      'Informes listos para inspección',
                      'Alertas para evitar pérdidas antes de que ocurran',
                      'Hasta 5 locales',
                      'Soporte prioritario',
                    ].map((f) => (
                  ...
                  <p className="mt-5 text-xs text-[var(--text-muted)] italic">Convierte el diagnóstico en resultados reales cada mes</p>
                  <Link
                    href="/auth/register"
                    className="mt-5 block w-full rounded-lg bg-primary-600 py-3 text-center text-sm font-semibold text-white hover:bg-primary-700 transition-colors"
                  >
                    Empezar con Pro
                  </Link>
                </div>
              ...
              <FadeInOnScroll delay={300}>
                <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-8 shadow-sm hover:shadow-lg transition-shadow h-full flex flex-col">
                  <div className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-2">Para cadenas y grupos</div>
                  <h3 className="text-lg font-semibold text-[var(--text-primary)]">Business</h3>
                  <div className="mt-3 flex items-baseline gap-1">
                    <span className="text-4xl font-bold text-[var(--text-primary)]">99&euro;</span>
                    <span className="text-[var(--text-muted)]">/mes</span>
                  </div>
                  ...
                  <ul className="mt-6 space-y-3 text-sm text-[var(--text-secondary)] flex-1">
                    {[
                      ...
                      'Locales ilimitados',
                      'Benchmarking vs. restaurantes similares',
                  ...
                  <p className="mt-5 text-xs text-[var(--text-muted)] italic">Control total y eficiencia operativa a escala</p>
                  <Link
            ...
            <FadeInOnScroll delay={400}>
              <p className="text-center text-sm text-[var(--text-muted)] mt-10">
                La mayoría de restaurantes recuperan entre <strong className="text-[var(--text-secondary)]">3 y 10 veces el coste del plan</strong> en los primeros 3 meses.
